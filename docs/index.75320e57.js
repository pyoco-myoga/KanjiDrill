...
class Token {
    constructor(kind, fragment = null){
        this.kind = kind;
        this.fragment = fragment;
    }
}
function lexer(question) {
    let result = [];
    ...
    if (previous_token_end_index !== question.length - 1) result.push(new Token("others", question.substring(previous_token_end_index)));
    return result;
}
class DisplayType {
    constructor(display, underline = false){
...
function parser(tokens) {
    let question = [];
    let answer = [];
    for(let i = 0; i < tokens.length;)if (tokens[i].kind === TokenKind.LeftBrace) {
        ++i;
        // expected yomi
        ...
        // expected }
        ++i;
    } else question.push(new DisplayType(tokens[i++].fragment));
    return [
        question,
        answer
    ];
...
((numQuestion)=>{
    for(let i = 1; i <= numQuestion; ++i)(0, _jqueryDefault.default)("#inputs").append(`
            <tr>
...
(0, _jqueryDefault.default)("#download-button").click(()=>{
    ...
    (0, _jqueryDefault.default)("td.question").each((index, element)=>{
        const tokens = lexer((0, _jqueryDefault.default)(element).text());
...
},{}]},["dKxsn","jyKRr"], "jyKRr", "parcelRequirefa2e")
